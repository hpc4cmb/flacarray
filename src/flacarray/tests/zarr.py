...
class ZarrTest(unittest.TestCase):
    ...
    def test_array_write_read(self):
        ...
        for dt, dtstr, sigma in [
            ...
            filename = os.path.join(tmppath, "data_{dstr}.zarr")
            with ZarrGroup(filename, mode="w", comm=self.comm) as zf:
            ...
            if check != flcarr:
                ...
                print(f"FAIL on {dstr} FlacArray roundtrip to zarr", flush=True)
                self.assertTrue(False)
            else:
                ...
                if dtstr == "i32" or dtstr == "i64":
                    if not np.array_equal(output, input):
                        ...
                        print(f"input_{dtstr} = {input}", flush=True)
                        print("FAIL on {dtstr} array roundtrip to zarr", flush=True)
                        self.assertTrue(False)
                else:
