...
class HDF5Test(unittest.TestCase):
    ...
    def test_direct_write_read(self):
        ...
        for dt, dtstr, sigma in [
            ...
            check = None
            filename = os.path.join(tmppath, f"data_{dtstr}.h5")
            ...
            if dtstr == "i32" or dtstr == "i64":
                if not np.array_equal(check, input):
                    ...
                    print("FAIL on {dtstr} roundtrip to hdf5", flush=True)
    ...
    def test_array_write_read(self):
        ...
        for dt, dtstr, sigma in [
            ...
            filename = os.path.join(tmppath, "data_{dstr}.h5")
            with H5File(filename, "w", comm=self.comm) as hf:
                flcarr.write_hdf5(hf.handle)
            if self.comm is not None:
                self.comm.barrier()
            with H5File(filename, "r", comm=self.comm) as hf:
                check = FlacArray.read_hdf5(hf.handle, mpi_comm=self.comm)

            # Check array equality
            if check != flcarr:
                print(f"check_{dstr} = {check}", flush=True)
                print(f"flcarr_{dstr} = {flcarr}", flush=True)
                print(f"FAIL on {dstr} FlacArray roundtrip to hdf5", flush=True)
                self.assertTrue(False)
            else:
                ...
                if dtstr == "i32" or dtstr == "i64":
                    if not np.array_equal(output, input):
                        ...
                        print(f"input_{dtstr} = {input}", flush=True)
                        print("FAIL on {dtstr} array roundtrip to hdf5", flush=True)
                        self.assertTrue(False)
                else:
                    if not np.allclose(output, input, atol=1e-6):
                        print(f"output_{dtstr} = {output}", flush=True)
                        print(f"input_{dtstr} = {input}", flush=True)
